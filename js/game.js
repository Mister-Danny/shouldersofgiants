...
(function () {
  ...
  /* ── SOG namespace exports (consumed by game/ai.js, game/ui.js, game/board.js) ──
     SOG.state is owned by game/state.js (Pass 3a).
     SOG.board is owned by game/board.js (Pass 3b).
     SOG.input is owned by game/input.js (Pass 3c).
     This file's SOG.game retains only what sibling modules need that
     wasn't part of those extractions:
       • Board helpers that ai.js and ui.js reference via helpers.X
         (kept as pass-through aliases for backward compat).
       • isKenteProtected — ability state read by ai.js's Cortes scoring;
         lives in game.js until Pass 4 moves the abilities registry.
       • executeMove — cross-cutting move handler called by AI's
         runAiMovements and by the reveal phase's queued-move application.
       • refreshMoveableCards — exposed here so board.js's syncPlayerSlots
         can call it without forming a direct board → input edge; the
         function itself now lives in input.js (Pass 3c). */
  SOG.game = {
    shuffle:               shuffle,
    getSlotEl:             getSlotEl,
