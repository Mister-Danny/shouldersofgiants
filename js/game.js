...
(function () {
  ...
  /* ── SOG namespace exports (consumed by game/ai.js, game/ui.js, game/board.js) ──
     SOG.state is now owned by game/state.js (Pass 3a). Most board helpers
     are owned by game/board.js (Pass 3b). This file's SOG.game now exposes
     only what sibling modules need:
       • The board helpers that ai.js and ui.js reference (kept as pass-through
         aliases for backward compat; in a final cleanup pass we can have them
         import directly from SOG.board).
       • refreshMoveableCards, which board.js's syncPlayerSlots calls (the
         function still lives in game.js because it's an input/affordance
         concern; Pass 3c is where it moves out). */
  SOG.game = {
    shuffle:               shuffle,
    getSlotEl:             getSlotEl,
