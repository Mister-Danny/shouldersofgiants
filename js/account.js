...
window.SogAccount = (function () {
  ...
  /* ── Class join links ───────────────────────────────────────────────────
     A teacher shares <site>/?join=E233V7 (js/teacher-dashboard.js renders it)
     instead of dictating a code. The link carries exactly the same secret the
     typed code always was — classes are `get`-able by exact id and `list` is
     denied (firestore.rules), so a link can't be used to enumerate classes.
     Pure string parsing, no DOM/Firebase: callable from anywhere and unit
     testable. Accepts ?join= or #join= and normalizes like lookupClassCode. */
  function parseJoinCode(search, hash) {
    var re = /(?:^|[?&#])join=([^&#]+)/i;
    var m = re.exec(String(search || '')) || re.exec(String(hash || ''));
    if (!m) return '';
    var code = '';
    try { code = decodeURIComponent(m[1]); } catch (e) { code = m[1]; }
    code = code.trim().toUpperCase();
    // Class ids are the 6-char codes generated by the dashboard; anything else
    // is a mangled/typo'd link and is treated as "no join link" rather than
    // sent to Firestore as a junk document id.
  ...
  function signUpStudent(classInfo, username, passphrase, cb) {
    ...
    _tryLink(user, username, passphrase, 1, function (linkErr, creds) {
      ...
      // Passphrase on the student's own doc so their teacher can read it back
      // when the card gets lost — the roster view (js/teacher-dashboard.js) is
      // the only other reader firestore.rules allows (owning teacher via
      // teacherUid; no one else, including other teachers). It is a WORKING
      // credential for username@sog.invalid, deliberately stored in the clear
      // because there is no recovery path otherwise and a client cannot reset
      // another user's password. Scope of harm is game progress: these docs
      // carry no student PII. Omitted (rather than written null, which merge
      // would persist) on the alreadyLinked retry, where the real passphrase
      // was never captured and a null would wipe a good stored one.
      if (creds.passphrase) playerData.passphrase = creds.passphrase;
  ...
  /* ── Teacher Google sign-in (Phase 4, alongside email/password) ─────────
     ...
     Chromebooks, so a blocked/unsupported popup falls back to a redirect,
     with the invite code parked in sessionStorage across the round trip
     (sessionStorage, not localStorage: it is per-tab and short-lived, and an
  ...
  function _restoreProgressAfterLogin(uid, cb) {
    _db().collection('players').doc(uid).get().then(function (snap) {
      if (snap.exists && snap.data().progress && window.SaveState) {
        window.SaveState.applySnapshot(snap.data().progress);
      }
      ...
      cb();   // offline: stay signed in on whatever local progress this device has
    });
  }
  ...
  // Signed in as a Google user; decide returning-vs-new and finish the job.
  function _finishTeacherGoogle(userCred, inviteCode, cb) {
    var user  = userCred.user;
    var isNew = !!(userCred.additionalUserInfo && userCred.additionalUserInfo.isNewUser);

    _withTimeout(_db().collection('teachers').doc(user.uid).get(), WRITE_TIMEOUT_MS).then(function (snap) {
  ...
  /**
   * @param {string} inviteCode  required only for a Google account that has
   *   never signed in here before; ignored for returning teachers.
   * @param {function(err, result)} cb  result = { uid, email, displayName, existing }
   */
  function signInTeacherWithGoogle(inviteCode, cb) {
    var code = (inviteCode || '').trim().toUpperCase();
    firebase.auth().signInWithPopup(_googleProvider()).then(function (userCred) {
      ...
      if (popupFailed) {
        try { sessionStorage.setItem(PENDING_INVITE_KEY, code); } catch (e) {}
        firebase.auth().signInWithRedirect(_googleProvider()).catch(function (redirectErr) {
          console.error('[Account] Google redirect sign-in failed', redirectErr);
          cb(redirectErr, null);
        });
      ...
      console.error('[Account] Google sign-in failed', err);
      cb(err, null);
    });
  }

  /**
   * Call once at boot: finishes a redirect-based Google sign-in if one is
   * pending. cb(err, result) runs ONLY when there was something to finish, so
   * a normal page load stays silent.
   */
  function completeTeacherGoogleRedirect(cb) {
    if (!firebase.auth().getRedirectResult) return;
    firebase.auth().getRedirectResult().then(function (userCred) {
      if (!userCred || !userCred.user) return;   // nothing pending — ordinary load
      var code = '';
      try { code = sessionStorage.getItem(PENDING_INVITE_KEY) || ''; } catch (e) {}
